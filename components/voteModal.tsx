...
export function RecordedVoteModal({ onClose }: ModalProps) {
  const [email, setEmail] = useState("");
...
export function OtherVoteModal({ onClose }: ModalProps) {
  const [email, setEmail] = useState("");
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
      <div className="relative bg-[#141419] rounded-xl p-6 w-[90%] max-w-md text-center shadow-lg">
        ...
        <div>
          ...
          <button className="cursor-pointer mt-4 w-full px-4 py-3 bg-[#4F46E5] rounded-full text-white text-sm hover:opacity-90 transition">
            Submit
          </button>
        </div>
      </div>
